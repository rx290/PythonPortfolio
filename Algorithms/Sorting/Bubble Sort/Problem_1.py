...
is a simple sorting algorithm that repeatedly steps through the list to be sorted, compares each pair of adjacent items and swaps them 
...
The algorithm, which is a comparison sort, is named for the way smaller elements "bubble" to the top of the list. Although the algorithm is simple, 
...
expected_result =[]
ele = sample_data[0]
for i in range(len(sample_data)-1,0,-1):
    for j in range(i):
        if sample_data[j] >= sample_data[j+1]:
            sample_data[j],sample_data[j+1] = sample_data[j+1],sample_data[j]
        else:
            pass
